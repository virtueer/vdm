...
export default function App() {
  ...
  const [formatModalTarget, setFormatModalTarget] = useState<{
    id?: string;
    url: string;
    title?: string;
  } | null>(null);

  const handleNewYouTubeDownloadWithoutFormat = useCallback(
  ...
  const handleUpdateItemStatus = (id: string, status: string, msg: string) => {
    setDownloads((prev) =>
      prev.map((d) => (d.id === id ? { ...d, status, speed: '', statusMsg: msg } : d))
    );
  };
  ...
  return (
    <div className="flex flex-col h-full bg-background text-foreground transition-colors">
      ...
      <main className="flex-1 overflow-hidden flex flex-col relative">
        ...
        >
          <div
            className={`flex-1 flex flex-col min-h-0 ${activeTab === 'downloads' ? '' : 'hidden'}`}
          >
            <DownloadsView
              downloads={downloads}
          ...
          <div
            className={`flex-1 flex flex-col min-h-0 ${activeTab === 'settings' ? '' : 'hidden'}`}
          >
      ...
      {itemToDeleteObj && (
        <DeleteModal item={itemToDeleteObj} onClose={() => setItemToDelete(null)} />
      )}
      {logModalItem && (
        <ProcessLogModal
          downloadId={logModalItem}
          downloadLogs={downloadLogs}
