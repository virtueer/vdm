import { Check, Copy, FileVideo, Terminal } from 'lucide-react';
    ...
    <Card className="overflow-hidden transition-all hover:shadow-md relative group flex flex-col">
      <CardContent className="p-4 flex flex-col gap-2.5">
        <div className="flex items-center gap-3">
          <div
            className={`p-2 rounded-lg ${item.status === 'completed' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-muted text-muted-foreground'}`}
          >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                {item.formatId && (
                  <Badge
                    variant="outline"
                    className="text-[10px] shrink-0 border-orange-500/50 text-orange-600 dark:text-orange-400 font-mono"
                  >
                    -f {item.formatId}
                  </Badge>
                )}
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {logCount > 0 && (
                <button
                  onClick={onViewLogs}
                  className="flex items-center gap-1 text-[10px] text-orange-500 hover:text-orange-400 hover:bg-orange-500/10 px-1.5 py-0.5 rounded transition-colors"
              ...
              {stats.avgSpeed && (
                <>
                  <span className="text-muted-foreground/30">•</span>
                  <span className="text-xs text-muted-foreground" title="Average Speed">
                    Avg: {stats.avgSpeed}
                  </span>
                </>
              )}
              {stats.duration && (
                <>
                  <span className="text-muted-foreground/30">•</span>
                  <span className="text-xs text-muted-foreground" title="Elapsed Time">
                    ⌚ {stats.duration}
                  </span>
                </>
              )}
        ...
        {(showProgressInfo || item.status === 'error' || item.status === 'cancelled') && (
          <div className="flex items-center gap-3">
            <Progress
              value={progress}
              className={`h-1.5 flex-1 ${item.status === 'error' ? 'opacity-50' : ''}`}
            />
