import { Check, Loader2, SlidersHorizontal, XCircle } from 'lucide-react';
...
import { ResumeDownload, SetDownloadFormat } from '../../../bindings/vdm/app';
...
export function YouTubeFormatModal({
  target,
  downloads,
  onManualDownload,
  onClose,
}: YouTubeFormatModalProps) {
  ...
  const [formatCategory, setFormatCategory] = useState<'all' | 'combined' | 'video' | 'audio'>(
    'all'
  );
  ...
  const applySelectedFormat = () => {
    ...
    if (
      targetFormat &&
      targetFormat.vcodec !== 'none' &&
      targetFormat.acodec === 'none' &&
      autoAppendAudio
    ) {
    ...
    if (target.id) {
      SetDownloadFormat(target.id, finalFormat)
        .then(() => {
    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-4xl h-[80vh] p-0 gap-0 overflow-hidden flex flex-col">
        <DialogHeader className="px-6 py-4 border-b bg-muted/30">
          ...
          <p
            className="text-xs text-muted-foreground truncate max-w-xl mt-0.5"
          ...
          {formatLoading ? (
            <div className="h-full flex flex-col items-center justify-center gap-3 text-muted-foreground">
              ...
              <span className="text-sm font-medium">
                Extracting available formats using yt-dlp...
              </span>
            </div>
          ) : formatError ? (
        ...
        <DialogFooter className="px-6 py-4 border-t bg-card flex items-center justify-between sm:justify-between">
          <div className="text-xs text-muted-foreground">
            {selectedFormatId ? (
              <span>
                Selected Format:{' '}
                <strong className="text-orange-500 font-mono">{selectedFormatId}</strong>
                {autoAppendAudio &&
                  formatsList.find((f) => f.formatId === selectedFormatId)?.vcodec !== 'none' &&
                  formatsList.find((f) => f.formatId === selectedFormatId)?.acodec === 'none' && (
                    <span className="text-blue-500 font-medium">
                      {' '}
                      (+bestaudio auto-merged into MP4)
                    </span>
                  )}
              </span>
            ) : (
