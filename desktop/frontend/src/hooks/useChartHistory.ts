...
import { formatChartSpeed, parseSizeToMB, parseSpeed } from '../utils/chartUtils';
...
function getStoredHistory(
  downloadId?: string
): { history: any[]; accumulatedTime: number } | undefined {
...
export function useChartHistory({
  ...
  const latestDownloadedRef = useRef<{ dl?: string; tot?: string; pct?: number }>({
    dl: downloadedSize,
    tot: totalSize,
    pct: progress,
  });
  ...
  useEffect(() => {
    if (!isDownloading) {
      ...
      if (onStatsUpdateRef.current) {
        const totalSecs =
          elapsedSecs !== undefined && elapsedSecs > 0 ? elapsedSecs : accumulatedTimeRef.current;
