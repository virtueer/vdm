...
export function sortFormats(formatsList: YouTubeFormat[], formatCategory: 'all' | 'combined' | 'video' | 'audio', formatSortField: FormatSortField | null, formatSortAsc: boolean): YouTubeFormat[] {
    ...
    return [...filtered].sort((a, b) => {
        ...
        if (formatSortField === 'resolution') {
            ...
            if (!isNaN(intA) && !isNaN(intB)) { valA = intA; valB = intB; }
        }
        ...
        return formatSortAsc ? String(valA).localeCompare(String(valB)) : String(valB).localeCompare(String(valA));
    });
}
